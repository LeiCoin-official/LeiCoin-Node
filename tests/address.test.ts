...
describe('address_testing', () => {
    test('address_enoding_and_decoding', () => {
        ...
        const address = Address32.fromPrivateKey(PX.from(0), privateKeyHex);

        const hashData = Crypto.sha256("0123456789abcdef");

        const signature = Crypto.sign(hashData, PX.from(0), privateKeyHex);
    ...
    test('coinbase_address_gettting', () => {
        ...
        const address = AddressHex.fromPrivateKey(PX.from(0), privateKeyHex);
